"""
Auto-generated test script from Excel
Test Case: TC010 - Cart Badge Count
Generated: 2026-03-21 11:02:04
"""

import os
import sys
import pytest
import requests

sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..'))

from python_framework.base.base_page import BasePage


class Test_TC010:
    """Cart Badge Count"""

    @pytest.fixture(autouse=True)
    def setup(self, driver):
        self.driver = driver
        self.page = BasePage(driver)

    @pytest.mark.regression
    def test_cart_badge_count(self):
        """Execute: Cart Badge Count"""
        page = self.page

        # Step 1: navigate
        page.navigate_to("https://www.saucedemo.com/v1/")

        # Step 2: type
        page.type_text("id", "user-name", "standard_user")

        # Step 3: type
        page.type_text("id", "password", "secret_sauce")

        # Step 4: click
        page.click("id", "login-button")

        # Step 5: wait
        page.wait_seconds(2)

        # Step 6: click
        page.click("css", ".inventory_item:nth-child(1) .btn_inventory")

        # Step 7: wait
        page.wait_seconds(1)

        # Step 8: verify_text
        page.verify_text("css", ".shopping_cart_badge", "1")

        # Step 9: click
        page.click("css", ".inventory_item:nth-child(2) .btn_inventory")

        # Step 10: wait
        page.wait_seconds(1)

        # Step 11: verify_text
        page.verify_text("css", ".shopping_cart_badge", "2")

        # Step 12: click
        page.click("css", ".inventory_item:nth-child(3) .btn_inventory")

        # Step 13: wait
        page.wait_seconds(1)

        # Step 14: verify_text
        page.verify_text("css", ".shopping_cart_badge", "3")

        # Step 15: take_screenshot
        page.take_screenshot("TC010_cart_badge_3_items")

